...
contains 
  real function fn(p)
    ...
    fn = 100*(x1**2-x2**2)**2+(1-x1)**2 + 90*(x3**2-x4)**2
  ...
  function grad(p) Result(del)
    ...
    del(1) = -2*(1-x1) + 400*x1*(x1**2-x2**2)
    del(2) = -20.2*(1-x2)-400*x2*(x1**2-x2**2)-19.8*(1-x4)
  ...
  function hessian(p) result(hess)
    ...
    do i = 1,n/4
       ...
       hess(1,1) = 2+800*x1**2+400*(x1**2-x2**2)
       hess(1,2) = -800*x1*x2
       hess(1,3) = 0
       hess(1,4) = 0

       hess(2,1) = -800*x1*x2
       hess(2,2) = 20.2+800*x2**2-400*(x1**2-x2**2)
       hess(2,3) = 0
       hess(2,4) = 19.8
